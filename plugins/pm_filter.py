    ...
    if imdb and imdb.get('poster'):
        try:
            if message.chat.id == SUPPORT_CHAT_ID:
                ...
                logger.exception(e)
                fek = await message.reply_photo(photo=imdb.get('url'), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
                try:
                    if settings['auto_delete']:
                        await asyncio.sleep(300)
                        await fek.delete()
            ...
            fuk = await message.reply_photo(photo=imdb.get('url'), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            try:
                if settings['auto_delete']:
                    await asyncio.sleep(300)
                    await fuk.delete()
