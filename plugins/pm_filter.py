...
#        i, movie = query.data.split('#')
        ...
        await query.answer("votes=imdb['votes'] {votes=imdb['votes']}, imdb['votes'] {imdb['votes']} {cap}, {cap=cap}", show_alert=True)
...
async def auto_filter(client, msg, spoll=False):
    ...
    sea = message.text
    message = msg
    cap = IMDB_TEMPLATE.format(query=sea, title=imdb['title'],votes=imdb['votes'])
...
#                await hmm.delete()                                                                                        
    ...
    try:
        ...
        if settings['auto_delete']:
            btn.insert(0, 
                [
                    InlineKeyboardButton(f'♻️ ɪɴꜰᴏ', 'im'),
                    InlineKeyboardButton("Languages", callback_data=f"languages#{search.replace(' ', '_')}#{key}"),
                    InlineKeyboardButton(f'ᴛɪᴘs​ ⚜', 'tips')
                ]
            )
